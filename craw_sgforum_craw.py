...
def write_urls():
    ...
    with open('crawled_urls.sgforums.txt','wa+') as furl:
...
def parse_title(title_url, fout_combo, fout_short, fout_all):
    ...
    try:
        ...
        while title_page_counter <= num_pages:
                page_url = title_url + '?page=%d'%title_page_counter
...
def parse_title_list(global_page_url, fout_combo, fout_short, fout_all):
    ...
    try:
        ...
        title_table = bsObj.find('table', {'class': 'topics'})
        topic_list = title_table.findAll('tr', {'class': 'topic'})
        ...
        while title_list_counter < num_titles:
            ...
            topic_posts = int(target_topic.find('td', {'class': 'posts'}).string)
            if topic_posts > 0:
                L.info('%d posts to be crawled'%topic_posts)
                title_url = target_topic.find('a', {'class': 'entry-title'}, href=True)
                title_url = SGFORUMS_URL_PAGE + title_url['href']
...
def parse_topic(target_urls, fout_combo, fout_short, fout_all):
    ...
    while global_counter < len(target_urls):
        ...
        # start_url = 'http://sgforums.com/forums/2852'
        ...
        try :
            ...
            while global_page_counter <= num_pages:
                page_url = start_url + '?page=%d'%global_page_counter
...
if __name__ == "__main__":
    ...
    p.add_argument("-s", "--batch_size", dest="batch_size", help="batch_size", type=int, default=100)
    ...
    start_site = ('SgForums-%d'%batch, SGFORUMS_URL_START)
